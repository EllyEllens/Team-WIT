...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // Controleer de gegevens van studenten in de database
    $sql = "SELECT * FROM studenten WHERE email = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($wachtwoord, $user['wachtwoord'])) {
            ...
            // Redirect afhankelijk van het e-mailadres of de rol van de gebruiker
            if (strpos($email, '@student.natin.sr') !== false) {
                // Als de e-mail eindigt op @student.natin.sr, ga naar dashboard.html
                header("Location: student.start.html");
                exit();
            } elseif ($user['rol'] === 'docent') {
                // Als de gebruiker een docent is, ga naar dashboard.html
                header("Location: docent.dashboard.html");
                exit();
            } elseif ($user['rol'] === 'admin') {
                // Als de gebruiker een admin is, ga naar admin.php
                header("Location: admin.php");
                exit();
            } else {
                // Als geen van de bovenstaande gevallen, ga naar welkom.php
